...
#else
   // _mosquitto_log_printf(mosq, MOSQ_LOG_ERR, "OpenSSL Error:---------------------------------------");
...
//    printf("---------mosquitto_packet_write\n");
  //  _mosquitto_log_printf(NULL, MOSQ_LOG_INFO, "-------_mosquitto_packet_write ");
...
#endif
{
 ...
 //   printf("---------mosquitto_packet_read\n");
//    _mosquitto_log_printf(NULL, MOSQ_LOG_INFO, "-------_mosquitto_packet_read ");
	if(!mosq) return MOSQ_ERR_INVAL;
	if(mosq->sock == INVALID_SOCKET) return MOSQ_ERR_NO_CONN;
	if(mosq->state == mosq_cs_connect_pending){
		return MOSQ_ERR_SUCCESS;
